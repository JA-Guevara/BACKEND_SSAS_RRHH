...
class RequestEmailVerification:
    ...
    async def execute(self, email: str, empresa_slug: str) -> tuple[str, str] | None:
        ...
        if not user or not user.is_active or user.email_verified or not user.empresa_id:
            return None
        ...
        await self.token_repository.save_email_verification_token(
            user.id, user.empresa_id, str(uuid4()), self.token_service.fingerprint(raw_token),
