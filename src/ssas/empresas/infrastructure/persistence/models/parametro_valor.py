...
class ParametroValorModel(Base):
    ...
    __table_args__ = (
        CheckConstraint("vigente_hasta IS NULL OR vigente_hasta >= vigente_desde", name="chk_parametro_valor_fechas"),
        Index("idx_parametro_valor_empresa_id", "empresa_id"),
        Index("idx_parametro_valor_parametro_id", "parametro_legal_id"),
    )
    ...
    parametro_legal_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey("parametro_legal.id", ondelete="CASCADE"), nullable=False)
