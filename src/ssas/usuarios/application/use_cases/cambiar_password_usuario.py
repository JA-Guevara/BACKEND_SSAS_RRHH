...
class CambiarPasswordUsuario:
    ...
    async def execute(
        self, user_id: str, empresa_id: str, new_password: str, must_change: bool = True
    ):
        ...
        updated = await self.repository.set_password(
            user_id, empresa_id, self.password_hasher.hash(new_password), must_change
        )
